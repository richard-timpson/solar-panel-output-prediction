...
The second transfer learning approach is an example of “zero shot” learning. (TODO: Explain what zero shot learning is in words. Not obvious from math)For each site, we train on $X^{s^c}_{Train} = X_{Train} / X^s_{Train}$ and then evaluate the model $M^{s^c}_Z$ on $Y^s_{Test}$ to obtain $Err(s, M^{s^c}_Z$. Zero-shot is clearly the model to do well on(TODO: What does this sentence mean?). 
