...
To demonstrate a trend amongst all models for all systems, we compare each model to the worst performing model and average those results across all different systems. The worst performing model, on average, is weather only. Here we will define a global model set
\[\mathbb{M} = \left\{ M_{IR}, M_{IR+T}, M_{W+T}, M_{W+IR}, M_{all}, M_{Z}, M_{Int} \right\}\] 
which represents an aggregate of the different models error results across the different systems. We’ll define $RMSE(s, M)$ as a function that will obtain the RMSE of a given site and given model. Then 
\[
